...
export function PlacesMap({ places, selectedPlaceIndex, onPlaceSelect, hoveredPlaceIndex, showRoute = false, onRouteToggle }: PlacesMapProps) {
  const mapRef = useRef<any>(null)
  const { routeState, calculateRoute, toggleRouteVisibility } = useRouteCalculation()
  ...
  // Calculate route when showRoute is enabled and places change
  useEffect(() => {
    if (showRoute && validPlaces.length >= 2) {
      calculateRoute(validPlaces)
    }
  }, [showRoute, validPlaces, calculateRoute])
